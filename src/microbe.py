...
class Microbe():
    ...
    def microbe_osmoproduction_rate(self,OsmoGenes_df):
        ...
        Input:
            OsmoGenes_df: taxon-specific available genes for osmolyte;dataframe (taxon * genes); from the above microbe_osmolyte_gene() method
    ...
    def microbe_mortality(self,fb_grid):
        ...
        Return:
            death_rate: spatal taxon-specific basal death probability
            beta:       spatial beta
...
def microbe_osmo_psi(wp,alfa,wp_fc,wp_th):
    ...
    """
    Inducible osmolytes' production triggered when PSI declines to a **threshold** value
    At water potential of wp_th, the production reaches maxima
    --------------------------
    Inputs:
